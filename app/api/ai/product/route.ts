...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!affiliateUrl && (!productNameInput || !sourceInput)) {
      return NextResponse.json({ error: 'Paste an affiliate/product URL, or switch to Manual mode.' }, { status: 400 })
    }
    ...
    if (affiliateUrl && !manual) {
      try { extracted = await extractFromPage(affiliateUrl) }
      catch (error: any) { return NextResponse.json({ error: error?.message || 'Could not read this product page. Try Manual mode.' }, { status: 422 }) }
    }
    ...
    const source = clean([
      ...
      extracted?.pageText,
      extracted?.sku ? `Product ID/SKU: ${extracted.sku}` : '',
      extracted?.price ? `Current price shown: ₹${extracted.price}` : '',
      extracted?.oldPrice ? `Previous/list price shown: ₹${extracted.oldPrice}` : '',
    ].filter(Boolean).join('\\n\\n'), 16000) || sourceInput
...
No markdown. No code fences.`
    ...
    return NextResponse.json({
      ...
      source_url: extracted?.finalUrl || affiliateUrl || null,
      affiliate_url: affiliateUrl || null,
      image_url: extracted?.image || null,
      price: extracted?.price ? Number(extracted.price) : null,
      old_price: extracted?.oldPrice ? Number(extracted.oldPrice) : null,
      currency: extracted?.currency || 'INR',
      asin: extracted?.sku || null,
