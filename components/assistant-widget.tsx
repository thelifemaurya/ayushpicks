...
export default function AssistantWidget() {
  ...
  const [messages, setMessages] = useState<{ role:'assistant'|'user'; text:string }[]>([{ role:'assistant', text:'Hi! I’m the AYUSHPICKS guide. Ask me where to find something or how the site works.' }])
  ...
  async function send(text = input) {
    ...
    try {
      const response = await fetch('/api/ai/assistant', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ message:value, pathname:window.location.pathname }) })
  ...
  return <>
    {open && <div className="siteAssistantPanel"><div className="siteAssistantHead"><div><strong>AYUSHPICKS Guide</strong><span>Ask about the site</span></div><button onClick={()=>setOpen(false)} aria-label="Close assistant"><X size={17}/></button></div><div className="siteAssistantMessages">{messages.map((m,i)=><div key={i} className={`siteAssistantMsg ${m.role}`}>{m.text}</div>)}{busy&&<div className="siteAssistantMsg assistant">Thinking…</div>}</div>{messages.length===1&&<div className="siteAssistantQuick"><button onClick={()=>send('Where can I find products?')}>Find products</button><button onClick={()=>send('Where are the buying guides?')}>Buying guides</button><button onClick={()=>send('How does AYUSHPICKS work?')}>How it works</button></div>}<form className="siteAssistantInput" onSubmit={e=>{e.preventDefault();send()}}><input value={input} onChange={e=>setInput(e.target.value)} placeholder="Ask me anything about the site…" aria-label="Ask the site assistant"/><button disabled={busy||!input.trim()} aria-label="Send"><Send size={16}/></button></form></div>}
